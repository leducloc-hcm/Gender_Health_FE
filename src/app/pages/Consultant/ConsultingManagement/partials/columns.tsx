...
interface ConsultingColumns {
  onInviteLink: (historyId: number, meetingLink: string, meetingPlatform: string) => void
}

export const getConsultingColumns = ({
  onInviteLink
}: ConsultingColumns): ColumnDef<ConsultantManagementResponse>[] => {
  return [
    {
        ...
        <div className='flex justify-center px-2'>
          <div className='flex items-center space-x-2'>
            <Button
              ...
              size='sm'
              onClick={() => {
                onInviteLink(row.original.id, row.original.meetingLink || '', row.original.meetingPlatform || '')
              }}
