...
import { ArrowUpDown, CheckCircle, FileText, RotateCcw, TestTube, User } from 'lucide-react'

interface StiColumnsProps {
  onPscVisited: (id: number) => void
  onSampleCollected: (id: number) => void
  onReportDate: (id: number) => void
  onRollback: (id: number, status: string) => void
...
export const getStiColumns = ({
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const canRollback = () => {
        return ['PSC_VISITED', 'COLLECTED', 'REPORT_READY', 'RESULT_AVAILABLE'].includes(item.status)
      }

      const nextAction = getNextAction()
      const rollbackStatus = getRollbackStatus()
      const rollbackLoading = actionLoading[`rollback-${item.id}`]

      if (!nextAction && !canRollback()) {
        return (
        ...
        <div className='flex justify-center space-x-2'>
          ...
          {nextAction && (
            <Button
              ...
              disabled={actionLoading[nextAction.loadingKey]}
              className={`${nextAction.bgColor} text-white transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 min-w-[120px]`}
              size='sm'
            >
              {actionLoading[nextAction.loadingKey] ? (
          ...
          {canRollback() && rollbackStatus && (
            <Button
              onClick={() => onRollback(item.id, rollbackStatus)}
              disabled={rollbackLoading}
              variant='outline'
              className='border-red-300 text-red-600 hover:bg-red-50 hover:border-red-400 min-w-[100px]'
              size='sm'
            >
              {rollbackLoading ? (
                <div className='flex items-center space-x-2'>
