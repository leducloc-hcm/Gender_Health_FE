...
export default function TypeOfTestManagement() {
  ...
  const columns: ColumnDef<TestTypeItem>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className='truncate text-sm text-gray-700 max-w-xs text-start'>{row.original.name || 'No name'}</div>
      )
    },
    {
      ...
      cell: ({ row }) => (
        <div
          ...
          title={row.original.description || 'No address'}
        >
          {row.original.description || 'No address'}
        </div>
      )
    },
    {
